...
class ZbwRigTestMethods(unittest.TestCase):
    ...
    def test_linear_interpolate_scalar(self):
        ...
        self.assertEquals(rig.linear_interpolate_scalar(A, B, .5), 2)
        self.assertEquals(rig.linear_interpolate_scalar(A, B, .0), 0)
        self.assertEquals(rig.linear_interpolate_scalar(A, B, 1), 4)
