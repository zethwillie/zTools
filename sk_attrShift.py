...
def sk_attrShiftUI():
    ...
    mainWindow = mc.window(winInfo.ui(), title=winInfo.windowName() + '             ' + winInfo.version(), sizeable=1,
                           minimizeButton=0, maximizeButton=0, widthHeight=[width, height])
    formLayout_1 = mc.formLayout(numberOfDivisions=200)
    rowLayout_1 = mc.rowLayout(numberOfColumns=2)
    button_1 = mc.button(label='UP', command='sk_attrShift.sk_attShiftProc(1)', height=40, width=100,
                         backgroundColor=[.5, .5, .5])
    button_2 = mc.button(label='DN', command='sk_attrShift.sk_attShiftProc(0)', height=40, width=100,
                         backgroundColor=[.5, .5, .5])
