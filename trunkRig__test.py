...
class VFK_UI(qg.QDialog):
    ...
    def create_vfk(self, name = "", numJoints=20.0, numControls=3.0, controlRadius = 4.0, 
        ...
        self._jRes(topJoint, endJoint, boneTranslateAxis= boneTranslateAxis, add= numJoints-2)
        ...
        for i in xrange(int(numControls)):
            ...
            flcl = self._parentFLCL(ctrl, surface[0])
            ...
            pmc.addAttr(ctrl[0], ln='position', min=0, max=1, dv=0, keyable=True)
            pmc.addAttr(ctrl[0], ln='falloff', min=min_falloff, max=1, dv=min_falloff, keyable=True)
            ...
            pmc.connectAttr(ctrl[0] + '.position', flcl[0] + '.parameterU')
            pmc.setAttr(ctrl[0] + '.position', paramU)
            
            fPos_plus = pmc.createNode('plusMinusAverage', n= name + 'fPosPlus_vfk_' + str(i+1))
            pmc.connectAttr(ctrl[0] + '.position', fPos_plus + '.input1D[0]', f=True)
            ...
            pmc.connectAttr(ctrl[0] + '.position', fPos_minus + '.input1D[0]', f=True)
            ...
            for f in (fPos_plus, fPos_minus):
                for j in xrange(len(joints)):
                    ...
                    pmc.connectAttr(ctrl[0] + '.position', lowerM + '.input1D[0]')
            ...
            for j in xrange(len(joints)):
                ...
                pmc.connectAttr(ctrl[0] + '.position', cond + '.firstTerm') # then use minus
                ...
                multFinal = pmc.createNode('multiplyDivide', n=  name + 'multFinal_j' + str(j+1) + '_c' + str(i+1))
                pmc.setAttr(multFinal + '.operation', 1)
                pmc.connectAttr(cond_neg + '.outColor', multFinal + '.input1')
                pmc.connectAttr(ctrl[0] + '.rotate', multFinal + '.input2')
                
                pmc.connectAttr(multFinal + '.output', name + jointGroupPrefix + 'j' + str(j+1) + '_c' + str(i+1) + '.rotate')
    ...
    # INTERNAL FUNCTIONS
    ####################
    def _jRes(self, parentJoint, endJoint, boneTranslateAxis = '.tx', add=2.0):
        pmc.undoInfo(openChunk=True)
        ...
        for i in xrange(int(round(add))):
            newJoint = pmc.joint(name=(str(parentJoint) + "_jResJoint_" + str(i+1)), radius = (boneRadius*2))
            mc.select(cl=True)
            pmc.parent(newJoint, parentJoint, relative=True)
            mc.setAttr(newJoint + boneTranslateAxis, (boneLength/(add + 1)))
            if i > 0:
                pmc.parent(newJoint, (str(parentJoint) + "_jResJoint_" + str(i)), relative = True)
            if i == add - 1:
    ...
    def _parentFLCL(self, constrained_obj, geo, deleteCPOMS=1):
        """
        Parents object to follicle at closest point on surface or mesh. 
        Select child transform, then select mesh to hold parent follicle. 
        Mesh must have valid UVs.
        """
        pmc.undoInfo(openChunk=True)
        
        shape_type = pmc.nodeType(pmc.listRelatives(geo, shapes=True, children=True))

        if shape_type == 'mesh':

            cpom = pmc.createNode('closestPointOnMesh', n='cpom_flcl_' + geo)
            mc.connectAttr(geo + '.outMesh', cpom + '.inMesh')
            obj_mtx = pmc.xform(constrained_obj, q=True, m=True, ws=True)
            pmc.setAttr(cpom + '.inPosition', [obj_mtx[12], obj_mtx[13], obj_mtx[14]])

            flclShape = pmc.createNode('follicle', n='flclShape')
            ...
            mc.connectAttr(geo + '.outMesh', flclShape + '.inputMesh')
            mc.setAttr(flclShape + '.simulationMethod', 0)

            u = mc.getAttr(cpom + '.result.parameterU')
            v = mc.getAttr(cpom + '.result.parameterV')
            ...
            print 'Must select either mesh or surface with valid UVs.'
            
        pmc.undoInfo(closeChunk=True)
        
    def _jntDBL(self, controls):
    ...
    def _testBind(self):
        ...
        ###### set attrs to test, delete later #######
        pmc.select(cl=True)
        bindJoints = pmc.ls(type='joint')

        pmc.skinCluster(bindJoints, 'trunk_geo', mi=3)
